...
from ml.mlp.model import MLP


def load_weights(model, path="ml/mlp/mlp_mnist.npz"):
...
def evaluate():
    ...
    try:
        load_weights(model)
        print("Loaded saved weights from ml/mlp/mlp_mnist.npz")
    except FileNotFoundError:
        print("No saved weights found. Training from scratch first...")
        from ml.mlp.train import train as do_train
        do_train()
        load_weights(model)
